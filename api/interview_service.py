...
Uses the openai SDK with the Emergent LLM proxy (same approach as cv_service.py).
"""
...
TARGET_TURN_COUNT_MIN = 6
TARGET_TURN_COUNT_MAX = 8
MAX_ANSWER_CHARS = 4000
MAX_CV_SUMMARY_CHARS = 6000
MAX_TTS_INPUT_CHARS = 1500
...
class _Chat:
    """Minimal stateful conversation wrapper over the openai SDK."""

    def __init__(self, api_key: str, system_message: str, model: str) -> None:
        self._model = model
...
def _system_message(
    ...
    focus_areas = "; ".join(target.focus_areas) if target.focus_areas else ("ei eritelty" if language == "fi" else "none provided")
    job_title = target.job_title or ("ei eritelty" if language == "fi" else "not specified")
    industry = target.industry or ("ei eritelty" if language == "fi" else "not specified")
    seniority = target.seniority or ("ei eritelty" if language == "fi" else "not specified")
    market = target.market or "Finland"
    jd = (target.job_description or "")[:2000]
...
<user_content>
{cv_summary[:MAX_CV_SUMMARY_CHARS]}
</user_content>
...
async def start_session(
    *,
    ...
    for model_name in LLM_MODELS_IN_ORDER:
        try:
            chat = _Chat(api_key=api_key, system_message=system_message, model=model_name)
